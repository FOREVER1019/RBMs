...
%% speed level of the node  sqr60_node_type.mat   directly find the fastest speed, because the relationship between the level and speed of different countries is not consistent
if type==1
    for n=1:Np
        if sum(ismember(edge_data(:,2),n) | ismember(edge_data(:,3),n))==0
            node_info(n)=0;
        else
...
%%  9 raster area betweenness of node  sqr60_rs1_node_local_r9_betweenness.mat
if type==2
...
%% 25 raster area betweenness of node  sqr60_rs1_node_local_r25_betweenness.mat
if type==3
    for r=1:length(raster_data(:,1))
...
%% 49 raster area betweenness of node  sqr60_rs1_node_local_r49_betweenness.mat
if type==4
    for r=1:length(raster_data(:,1))
...
%% the distance of node  sqr60_rs1_node_local_n2c_dist.mat£¬euc_dist.mat£¬net_dist.mat£¬net_time.mat
if type>=5
    for r=1:length(raster_data(:,1))
        local_raster_id=raster_data(abs(raster_data(:,2)-raster_data(r,2))<=1.1*log_step & abs(raster_data(:,3)-raster_data(r,3))<=1.1*lat_step,1);
        local_node=node_raster(ismember(node_raster(:,2),local_raster_id),1);
    ...
    %% the speed level of node sqr60_node_type.mat    directly find the fastest speed, because the relationship between the level and speed of different countries is not consistent
    if type==9
