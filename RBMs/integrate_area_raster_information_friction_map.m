function area_raster_info=integrate_area_raster_information_friction_map(org_node_data,org_edge_data,org_edge_str,net_raster_info)
...
min_log=min([net_raster_info.boundary_X org_node_data(:,NX)']);max_log=max([net_raster_info.boundary_X org_node_data(:,NX)']);
min_lat=min([net_raster_info.boundary_Y org_node_data(:,NY)']);max_lat=max([net_raster_info.boundary_Y org_node_data(:,NY)']);
...
new_raster_data=zeros(abs(right_x-left_x)*abs(up_y-bottom_y),6);%raster_id,longtitude,latitude,speed,population,type(1 or 2 or 3)
           ...
           %second check whether the raster is covered inside a boundary
            ...
            for b=2:length(bound_id)
                ...
                if any(inpolygon([rlog-log_step/2,rlog+log_step/2,rlog+log_step/2,rlog-log_step/2],[rlat-lat_step/2,rlat-lat_step/2,rlat+lat_step/2,rlat+lat_step/2],temp_X,temp_Y))
                   rid=rid+1;new_raster_data(rid,1:4)=[rid rlog rlat Data(yid,xid)];
                   if Data(yid,xid)<=0.003 % for those rasters with speed larger than 20km/h, use the walking speed
                      new_raster_data(rid,4)=0.012; %walking speed as the raster passing speed
                   end
                   break;
                end
            end
...
edge_raster=zeros(length(org_edge_data(:,1)),6);etag=0;%edge_id,raster_id,p_longtiude,p_latitude,dist_p_fnode,dist_p_tnode
...
for e=1:size(org_edge_data,1)
    edge_bound=[min(org_edge_str(e).X) min(org_edge_str(e).Y);max(org_edge_str(e).X) max(org_edge_str(e).Y)];
    ...
    if sum(raster_data(:,2)-log_step/2<=edge_bound(1,1) & raster_data(:,3)-lat_step/2<=edge_bound(1,2) & raster_data(:,2)+log_step/2>=edge_bound(2,1) & raster_data(:,3)+lat_step/2>=edge_bound(2,2))==0
        ...
        fid=net_raster_info.node_raster(org_edge_data(e,EF),2);tid=net_raster_info.node_raster(org_edge_data(e,ET),2);
        eff_raster(eff_raster(:,1)==fid,:)=[];eff_raster(eff_raster(:,1)==tid,:)=[];
        if ~isempty(eff_raster)
            edge_xy=[org_edge_str(e).X(1:end-1)' org_edge_str(e).Y(1:end-1)'];
            ...
            cross_raster=unique([new_edge_xy(:,6);new_edge_xy(:,7)]);
            cross_raster(cross_raster==0)=[];
            ...
            for n=1:length(cross_raster)
                ...
                if length(tempA)>1
                    ...
                    right_dist=org_edge_data(e,4)*0.001-left_dist;
...
% assign each raster the population according to GHS_POP data
[pop_Data,pop_R] = geotiffread("GHS_POP_E2015_GLOBE_R2019A_4326_30ss_V1_0\GHS_POP_E2015_GLOBE_R2019A_4326_30ss_V1_0.tif");
pop_lat_step=pop_R.CellExtentInLatitude;pop_log_step=pop_R.CellExtentInLongitude;
sample_step=10;
...
fids=net_raster_info.node_raster(org_edge_data(cross_edge_raster(:,1),EF),4);
tids=net_raster_info.node_raster(org_edge_data(cross_edge_raster(:,1),ET),4);
...
while ~isempty(cross_edge_raster)
    ...
    for k=1:length(unique_edge)
        temp_seq=sortrows(temp_edge_raster(temp_edge_raster(:,1)==unique_edge(k),1:6),5);fnode=org_edge_data(unique_edge(k),EF);tnode=org_edge_data(unique_edge(k),ET);
        edge_seq=[unique_edge(k) net_raster_info.node_raster(fnode,4) 0 0 0 0;temp_seq;unique_edge(k) net_raster_info.node_raster(tnode,4) 0 0 org_edge_data(unique_edge(k),EL)*0.001 0];
        for s=2:length(edge_seq(:,1)) 
            ...
            if isempty(temp_raster)
                ...
                if org_edge_data(edge_seq(s,1),EH)<org_edge_data(temp_raster(1,6),EH) || (org_edge_data(edge_seq(s,1),EH)==org_edge_data(temp_raster(1,6),EH) && abs(edge_seq(s,5)-edge_seq(s-1,5))<temp_raster(1,5))
...
[Data,R] = geotiffread('GHS_POP_E2015_GLOBE_R2019A_4326_30ss_V1_0\GHS_POP_E2015_GLOBE_R2019A_4326_30ss_V1_0.tif');
...
for r=1:length(raster_data(:,1))
    if raster_data(r,6)==3
        ...
        sample_fs=sample_fs(sample_fs(:,3)>=0.006,:);%only considering those friction surface points with speeds less than 10km/h, or more than 0.006 minute/meter
